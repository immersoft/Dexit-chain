...
contract BSCValidatorSet is IBSCValidatorSet, System {
    ...
    //         require(
    //             value.length == 32,
    //             "length of expireTimeSecondGap mismatch"
    //         );
    //         uint256 newExpireTimeSecondGap = BytesToTypes.bytesToUint256(
    //             32,
    //             value
    //         );
    //         require(
    ...
    //         require(
    //             newBurnRatio <= BURN_RATIO_SCALE,
    //             "the burnRatio must be no greater than 10000"
    //         );
    ...
    function unstakeValidator() external zeroAddress returns (bool) {
        ...
        require(
            highestValidators.length != 1 && isActiveValidator(staker),
        ...
        returns (bool)
    {
        ...
        require(amount > valInfo.amount, "Can't Withdraw More han Stake"); // Can't Unstake More Than Stake Amount
        ...
        valInfo.amount = valInfo.amount.sub(amount); // Update Amount
        valInfo.coins = valInfo.coins.sub(amount); // Update Coins

        uint256 balance = valInfo.amount; // get the updated balance stake self amount

        //Partial Withdraw
        if (balance > 0 && balance >= minimum_Stake_Amount) {
            valInfo.status = Status.Staked; // change status to staked
        } else if (balance == 0 && balance < minimum_Stake_Amount) {
            valInfo.status = Status.NotExist; // change status to staked
        }
        ...
        returns (bool)
    {
        ...
        require(amount > stakeInfo.amount, "Can't Withdraw More Than Stake"); // Can not withdraw more than stake
    ...
    function authchangevalues(bytes32 id) public {
        ...
        if (
            ...
            require(
                proposals[id].variable_value >= 2,
                "MaxValidators can't be less then 2"
            );
