...
// Function for staking validators
    function stakeValidator() external payable zeroAddress returns (bool) {
        ...
        } else if (highestValidators.length >= MaxValidators && !isTopValidator(staker) && valInfo.status != Status.Jailed) {
...
//Function to UnStakeValidator
    function unstakeValidator() 
        ...
        returns (bool) 
     {
        ...
        require(highestValidators.length != 3 && isActiveValidator(staker), "Can't Unstake, Validator List Empty");
...
//Function to UnStakeDelegator
        ...
        returns (bool)
    {
        ...
        require(unstakeamount > 0, "don't have any stake");
        require(highestValidators.length != 3,
...
//Function to WithdrawValidator staking after unstakeValidator
        ...
        returns (bool)
    {
        ...
        } else if(highestValidators.length < MaxValidators && highValidator != address(0) && validatorInfo[highValidator].status != Status.Jailed) {
        ...
        returns (uint256, uint256, address) 
    {
        ...
        for (uint256 k = 0; k < currentValidators.length; k++) {
            if (validatorInfo[currentValidators[k]].coins > highCoins &&
                ...
                validatorInfo[currentValidators[k]].status == Status.Staked && validatorInfo[currentValidators[k]].status != Status.Jailed
            ) {
