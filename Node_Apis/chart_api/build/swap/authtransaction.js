...
const authETH = (from, transactionHash) => __awaiter(void 0, void 0, void 0, function* () {
    let status = 'ok';
    from = Web3.utils.toChecksumAddress(from);
    const hash_value = yield Swap_ABI.methods.transactions(transactionHash).call();
    if (hash_value) {
        console.log("\nTransaction already processed");
        status = 'error';
        return status;
    }
    const transactionDetails = yield Web3.eth.getTransactionReceipt(transactionHash);
...
const authBSC = (from, transactionHash) => __awaiter(void 0, void 0, void 0, function* () {
    let status = 'ok';
    ...
    const transactionDetails = yield web3BSC.eth.getTransactionReceipt(transactionHash);
    console.log("Checking credentials for transactionDetails", transactionDetails);
    if (transactionDetails == null) {
        status = 'invalid hash';
        return status;
...
const authDXT = (from, transactionHash) => __awaiter(void 0, void 0, void 0, function* () {
    let status = 'ok';
    ...
    const transactionDetails = yield web3DXT.eth.getTransactionReceipt(transactionHash);
    console.log("Checking credentials for transactionDetails", transactionDetails);
    if (transactionDetails == null) {
        status = 'invalid hash';
        return status;
